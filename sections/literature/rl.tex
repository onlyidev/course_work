...
\subsubsection{\enquote{Andersono}}
\color{red} PLAN TO NOT INCLUDE
\color{black}
...
\mainModel{Agentas implementuotas kaip klasikiniai \acs{ml} algoritmai (konkrečiai dinaminis programavimas ir skirtumų laike (\textit{angl. temporal difference}) algoritmas).}
...
\perturbations{$A = \set{0,1,2,3}$ \vspace{5pt}}{
    ...
    \item Visos kompleksinės perturbacijos (\ref{sec:literature:perturbations:complex}), t.~y. tokios pačios, kaip ir \refFramework{MalFox} \glswhom{framework}.
}
\end{describeFramework}